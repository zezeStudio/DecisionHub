...
const translations = {
    ...
    "ko": {
        ...
        "info_title1": "사다리 타기의 유래: 동아시아의 지혜 '아미다쿠지'",
        "info_desc1": "사다리 타기는 동아시아에서 매우 대중적인 결정 도구로, 일본에서는 '아미다쿠지(阿弥陀籤)'라고 불립니다. 이 이름은 과거 사다리 모양이 아미다 부처의 후광(방사형 선)과 닮았다고 해서 붙여진 명칭입니다. 초기에는 부채꼴 모양의 선을 그어 제비를 뽑는 방식이었으나, 시간이 흐르며 오늘날 우리가 아는 수직선과 수평 다리 모양의 사다리 형태로 발전했습니다. 한국, 일본, 중국 등지에서 간식 내기나 역할 분담을 정할 때 없어서는 안 될 민속적인 놀이 문화로 자리 잡았으며, 단순한 게임을 넘어 집단의 화합을 다지는 유쾌한 의식으로 전해 내려오고 있습니다.",
        "info_title2": "사다리 타기는 왜 수학적으로 완벽할까? '치환'의 원리",
        "info_desc2": "사다리 타기가 공정한 이유는 수학의 '치환(Permutation)' 원리에 기반하기 때문입니다. 사다리의 가로선은 인접한 두 세로선의 위치를 맞바꾸는 역할을 합니다. 수학적으로 가로선 하나는 하나의 '전치(Transposition)'에 해당하며, 아무리 많은 가로선이 있더라도 출발점과 도착점은 항상 일대일 대응(Bijection)을 이룹니다. 즉, 어떤 중복이나 누락 없이 모든 참가자가 반드시 하나의 결과에 도달하게 되며, 무작위로 배치된 가로선 덕분에 각 결과에 도달할 확률은 모든 참가자에게 동일하게 보장됩니다. Zeze Hub의 사다리 타기는 이러한 수학적 완벽성을 알고리즘으로 구현하여 가장 신뢰할 수 있는 공정함을 제공합니다.",
        "info_title3": "사다리 타기 활용 팁: 더 재미있고 투명한 게임",
        "info_desc3": "<li><strong>공정한 내기의 정석:</strong> 참가자 수와 당첨 항목을 입력하면 가로선이 자동으로 무작위 생성됩니다. 사람이 직접 그릴 때 발생할 수 있는 조작의 우려가 전혀 없는 깨끗한 내기를 즐겨보세요.</li><li><strong>애니메이션의 긴장감:</strong> Zeze Hub의 사다리 타기는 결과가 선을 따라 실시간으로 그려지며 시각적 즐거움을 제공합니다. 친구들과 함께 화면을 보며 자신의 선이 어디로 꺾이는지 지켜보는 짜릿한 긴장감을 느껴보세요.</li><li><strong>일대일 대응 확인과 보안:</strong> '전체 결과' 버튼을 통해 모든 인원의 결과를 한눈에 투명하게 공개할 수 있습니다. 입력한 이름과 데이터는 서버에 저장되지 않으며 오직 현재 기기에만 임시 보관되므로 안심하고 사용하셔도 됩니다.</li>"
    }
};
